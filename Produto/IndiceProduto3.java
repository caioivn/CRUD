...
public class IndiceProduto3 {
    ...
    public IndiceProduto3(int o, String na) throws IOException {
    ...
    public int[] listaP(int c2) throws IOException {
    ...
    private int[] listaC2(int chave2, long pagina) throws IOException {
        ...
        // são ultrapassadas
        int i=0;
        while(i<pa.n && chave2>pa.chaves2[i]) {
            i++;
        }
        ...
        if(i<pa.n && pa.filhos[0]==-1 && chave2==pa.chaves2[i]) {

            // Cria a lista de retorno e insere as chaves secundárias encontradas
            ArrayList lista = new ArrayList();
            while(chave2<=pa.chaves2[i]) {
                
                if(chave2==pa.chaves2[i])
                    lista.add(pa.chaves1[i]);
                i++;
        ...
        if(i==pa.n || chave2<=pa.chaves2[i])
            return listaC2(chave2, pa.filhos[i]);
        else
            return listaC2(chave2, pa.filhos[i+1]);
    }
    ...
    // O método retorna a lista de elementos que possuem a chave (considerando
    // a possibilidade chaves repetidas)
    public int[] listaC(int c1) throws IOException {
    ...
    // Busca recursiva. Este método recebe a referência de uma página e busca
    // pela chave na mesma. A busca continua pelos filhos, se houverem.
    private int[] listaC1(int chave1, long pagina) throws IOException {
        ...
        if(i==pa.n || chave1<=pa.chaves1[i])
            return listaC1(chave1, pa.filhos[i]);
        else
            return listaC1(chave1, pa.filhos[i+1]);
    }
